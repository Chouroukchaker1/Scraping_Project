...
const ScrapeForm = ({ onScrape, loading, error, processing }) => {
  const [dates, setDates] = useState({
    start: new Date().toISOString().split('T')[0],
    end: new Date().toISOString().split('T')[0]
  });
  const [extractionComplete, setExtractionComplete] = useState(true); // Mode complet par défaut
  ...
  return (
    <div style={{
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Grand indicateur visuel quand extraction en cours */}
        {processing && (
          <div style={{
            marginTop: '1.5rem',
            padding: '1.5rem',
            ...
            <RefreshCw size={32} className="spinning" style={{ color: '#D97706' }} />
            <div>
              <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#92400E', marginBottom: '0.25rem' }}>
                🚀 EXTRACTION EN COURS...
              </div>
              <div style={{ fontSize: '0.9rem', color: '#78350F' }}>
                Veuillez patienter pendant que nous récupérons les données depuis TUNEPS
              </div>
            </div>
          </div>
        )}
...
const PendingList = ({ offres, onValidate, onUpdate, onDelete, processing }) => {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      const validated = response.data.validated_deleted || 0;
      alert(`✅ ${total} offres supprimées avec succès\n\n📋 Pending: ${pending}\n✅ Validées: ${validated}`);
              ...
              <tbody>
                {paginatedOffres.map((offre, idx) => (
                  <tr key={offre.reference || idx} style={{
                    ...
                    <td style={{ padding: '1rem', verticalAlign: 'top', color: '#475569' }}>{formatDate(offre.openingDate)}</td>
              ...
              <tbody>
                {paginatedTenders.map((tender, idx) => (
                  <tr key={tender.reference || idx} style={{
                    ...
                    <td style={{ padding: '1rem', verticalAlign: 'top', color: '#475569' }}>{formatDate(tender.openingDate)}</td>
                    <td style={{ padding: '1rem', textAlign: 'center', verticalAlign: 'top' }}>
                      <button 
...
const TunepsAO = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(''); // ✅ Stocker la date sélectionnée
  ...
  const fetchPendingOffres = async (dateFilter = null) => {
    try {
      ...
      if (response.data.success) {
        const offers = response.data.pending?.offres || response.data.data?.offers || [];
        setPendingOffres(offers);
        setProcessing(response.data.pending?.processing || response.data.scraping?.isProcessing || false);
      }
    } catch (err) {
  ...
  const handleScrape = async (params) => {
    ...
    // ✅ Stocker la date pour filtrage ultérieur
    ...
    try {
      ...
      if (response.data.success) {
        alert('✅ Scraping démarré avec succès! Les données vont s\'afficher...');
        setProcessing(true);

        // ✅ Rafraîchir TOUTES les offres pendant le scraping (sans filtre de date)
