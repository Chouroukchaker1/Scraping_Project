// src/pages/Benin.js - VERSION PROFESSIONNELLE COMPLÈTE
  ...
  const fetchHealth = async () => {
    try {
      ...
      if (response.data.error) {
        setError('⚠️ Service BÉNIN non disponible');
      } else {
        setError('');
      }
    } catch (err) {
      setHealth({ error: err.message });
      setError('❌ Erreur de connexion au serveur BÉNIN');
    }
  };
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{
          ...
          <h1 style={{
            ...
            🇹🇳 Scraper BÉNIN Tunisie
          </h1>
          <p style={{ color: '#64748B', fontSize: '1.1rem' }}>
            Extraction automatique des marchés publics tunisiens (BÉNIN)
          </p>
        </div>
