      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{
              padding: '0.75rem 1.5rem',
              background: loading || !tenders.length ? '#FCA5A5' : '#EF4444',
              color: 'white',
              border: 'none',
              borderRadius: '10px',
              fontSize: '0.95rem',
              fontWeight: '600',
              cursor: loading || !tenders.length ? 'not-allowed' : 'pointer',
              display: 'flex',
              alignItems: 'center',
              gap: '0.5rem',
              transition: 'all 0.2s'
...
const Niger = () => {
  ...
  const fetchTenders = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE}/jobs?status=all&limit=100`);
      if (response.data.success) {
        setTenders(response.data.jobs);
      }
    } catch (err) {
