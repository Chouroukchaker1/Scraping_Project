// src/pages/GIZ.js - VERSION PROFESSIONNELLE COMPLÈTE
    ...
    <div style={{
      ...
      <form onSubmit={handleSubmit}>
        <h2 style={{
          ...
          Scraping des Marchés Publics Tunisiens GIZ
        </h2>
...
const GIZ = () => {
  ...
  const fetchHealth = async () => {
    try {
      ...
      if (response.data.error) {
        setError('⚠️ Service GIZ non disponible');
      } else {
        setError('');
      }
    } catch (err) {
      setHealth({ error: err.message });
      setError('❌ Erreur de connexion au serveur GIZ');
    }
  };
  ...
  const handleScrape = async (formData) => {
    ...
    try {
      ...
      if (response.data.success) {
        alert('✅ Scraping GIZ démarré avec succès! Rafraîchissez dans quelques minutes.');
        setProcessing(true);
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{
          ...
          <p style={{ color: '#64748B', fontSize: '1.1rem' }}>
            Extraction automatique des marchés publics tunisiens (GIZ)
          </p>
        </div>
