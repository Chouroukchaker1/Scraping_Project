// src/pages/Pnud.js - VERSION PROFESSIONNELLE COMPLÈTE CORRIGÉE
import React, { useState, useEffect, useMemo } from 'react';
import axios from 'axios';
import { Search, Calendar, CheckCircle, Edit2, Trash2, RefreshCw, Play, AlertCircle, Globe, Send } from 'lucide-react';
import './Pnud.css';
    ...
    <div style={{
      ...
      <form onSubmit={handleSubmit}>
        <h2 style={{
          ...
          Rechercher des Appels d'Offres PNUD
        </h2>
        ...
        <div style={{
          ...
          ℹ️ Si aucune date n'est spécifiée, le scraping s'effectuera sur la période par défaut du PNUD
        </div>
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '1rem',
            background: loading ? '#94A3B8' : 'linear-gradient(135deg, #0066cc 0%, #0052a3 100%)',
            color: 'white',
            border: 'none',
            borderRadius: '10px',
            fontSize: '1rem',
...
const PendingList = ({ offres, onValidate, onUpdate, onDelete, processing, onPostAll, onDeleteAll }) => {
  ...
  const filteredOffres = useMemo(() => 
    offres.filter(o => 
      ...
      (o.description_fr && o.description_fr.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (o.description && o.description.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (o.country && o.country.toLowerCase().includes(searchTerm.toLowerCase()))
    ),
    [offres, searchTerm]
  );
  ...
  const handleDeleteAll = () => {
    if (!window.confirm(`⚠️ ATTENTION ⚠️\n\nSupprimer TOUTES les ${filteredOffres.length} offres en attente ?\n\nCette action est IRRÉVERSIBLE !`)) return;
    onDeleteAll();
  };
                  ...
                  <tr key={offre.reference || idx} style={{
                    ...
                    <td style={{ padding: '1rem', verticalAlign: 'top', maxWidth: '300px' }}>
                      <span style={{ color: '#475569' }}>
                        {(offre.description_fr || offre.description || '').substring(0, 120)}
                        {(offre.description_fr || offre.description || '').length > 120 ? '...' : ''}
                      </span>
                    </td>
                    <td style={{ padding: '1rem', verticalAlign: 'top' }}>
                    ...
                    <td style={{ padding: '1rem', verticalAlign: 'top', color: '#475569' }}>{offre.promoter || 'UNDP'}</td>
                    <td style={{ padding: '1rem', verticalAlign: 'top' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        {offre.externalUrl && (
                          <a 
                            href={offre.externalUrl} 
                            target="_blank" 
...
const ValidatedList = ({ tenders, loading }) => {
  ...
  const filteredTenders = useMemo(() => 
    tenders.filter(t => 
      ...
      (t.description_fr && t.description_fr.toLowerCase().includes(searchTerm.toLowerCase())) ||
              ...
              <tbody>
                {paginatedTenders.map((tender, idx) => (
                  <tr key={tender.reference || idx} style={{
                    ...
                    <td style={{ padding: '1rem', verticalAlign: 'top', maxWidth: '300px' }}>
                      <span style={{ color: '#475569' }}>
                        {(tender.description_fr || tender.description || '').substring(0, 120)}
                        {(tender.description_fr || tender.description || '').length > 120 ? '...' : ''}
                      </span>
                    </td>
                    <td style={{ padding: '1rem', verticalAlign: 'top' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Globe size={16} color="#059669" />
                        <span style={{ color: '#475569' }}>{tender.country || 'N/A'}</span>
                      </div>
                    </td>
                    <td style={{ padding: '1rem', verticalAlign: 'top', color: '#475569' }}>{formatDate(tender.expirationDate)}</td>
                    <td style={{ padding: '1rem', verticalAlign: 'top', color: '#475569' }}>{tender.promoter || 'UNDP'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const Pnud = () => {
  ...
  const fetchHealth = async () => {
    try {
      ...
      if (response.data.error) {
        setError('⚠️ Service PNUD non disponible');
      } else {
        setError('');
      }
    } catch (err) {
      setHealth({ error: err.message });
      setError('❌ Erreur de connexion au serveur PNUD');
    }
  };

  const fetchPendingOffres = async () => {
    try {
      const response = await axios.get(`${API_BASE}/pending_tenders?page=1&limit=1000`);
      if (response.data.status === 'success') {
        setPendingOffres(response.data.tenders || []);
      }
    } catch (err) {
  ...
  const handleScrape = async (dates) => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/scrape`, {
        date_debut: dates.start || null,
        date_fin: dates.end || null
      });
      
      if (response.data.success || response.status === 200) {
        alert('✅ Scraping PNUD démarré avec succès! Rafraîchissez dans quelques minutes.');
        setProcessing(true);
  ...
  const handleDeleteAll = async () => {
    try {
      ...
      if (response.data.success) {
        alert(`🗑️ ${response.data.deleted_count} offres supprimées avec succès`);
        setPendingOffres([]);
        fetchPendingOffres();
      } else {
        alert(`❌ Erreur: ${response.data.message}`);
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{
          ...
          <p style={{ color: '#64748B', fontSize: '1.1rem' }}>
            Extraction automatique des appels d'offres du Programme des Nations Unies
          </p>
        </div>
